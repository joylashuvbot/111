...
def extract_place_address_from_gmaps_url(url: str) -> str | None:
    """
    Agar havolada lat/lng topilmasa (masalan faqat CID/place-id bo'lsa),
    /maps/place/<manzil>/data=... qismidan haqiqiy manzil matnini ajratib oladi.
    Masalan:
    https://www.google.com/maps/place/Caravan+Restaurant,+317+N+Schmidt+Rd,+Bolingbrook,+IL+60440/data=...
    -> "Caravan Restaurant, 317 N Schmidt Rd, Bolingbrook, IL 60440"
    """
    try:
        ...
        # Rus tilidagi umumiy shahar nomlari ham bo'lishi mumkin (masalan "Эльджин, Иллинойс, США")
...
async def confirm_add(call: types.CallbackQuery, state: FSMContext):
    ...
    if lat is None:
        # 2) Faqat shu ham ishlamasa, umumiy shahar nomiga tushamiz
...
async def by_location(message: types.Message):
    ...
    # DEBUG: har bir joygacha masofani va lat/lng turini logga chiqaramiz
    for p in PLACES:
        d = haversine(lat, lng, p["lat"], p["lng"])
        print(f"[DEBUG] {p.get('name')} -> {d:.1f} km (lat={p.get('lat')!r}, lng={p.get('lng')!r})")
